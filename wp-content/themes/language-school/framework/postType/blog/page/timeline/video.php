	...
	<?php
		if (!post_password_required()) {
			if ($cmsmasters_post_video_type == 'selfhosted' && !empty($cmsmasters_post_video_links) && sizeof($cmsmasters_post_video_links) > 0) {
				...
				$attrs = array( 
					'preload'  => 'none', 
					'height'   => $video_size['cmsmasters-post-thumbnail']['height'], 
					'width'    => $video_size['cmsmasters-post-thumbnail']['width'] 
				);
				
				
				if (has_post_thumbnail()) {
					$video_poster = wp_get_attachment_image_src((int) get_post_thumbnail_id(get_the_ID()), 'cmsmasters-post-thumbnail');
				...
				echo '<div class="cmsmasters_video_wrap">' . 
					do_shortcode($wp_embed->run_shortcode('[embed width="' . $video_size['cmsmasters-post-thumbnail']['width'] . '" height="' . $video_size['cmsmasters-post-thumbnail']['height'] . '"]' . $cmsmasters_post_video_link . '[/embed]')) . 
				'</div>';
			} elseif (has_post_thumbnail()) {
				language_school_thumb(get_the_ID(), 'cmsmasters-post-thumbnail', true, false, true, false, true, true, false);
			}
		}
